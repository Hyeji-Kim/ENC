...
def Cost_Opt(Ctarget):
    ...
    Sel_top = 20 # number of validation set {1 : ENC-Model} {over 2 : ENC-Inf}
    Sel_metric = 2 # type of whole-layer accuracy metric {0 : Ac - combination} {1 : Am - val acc} {2 : Ap - PCA energy}
    # -------------------------------------------------------------------------

    # Initial Settings for Combinatorial Space ---------------------
    num_sub_group = 4 # maximum number of sub-groups in a top group  (vgg16 : 1 / res56 : 4)
    Sel_space = 0.1 # space margin {0 ~ 1.0} {0 : ENC-Map}
    Sel_margin = 0.005 # complexity margin
    Sel_density = 0.4 # average density ratio {0.0 ~ 0.99} 
    # --------------------------------------------------------------
